...
function textSaldoVacaciones(){
	...
	if($mostrar==1){
		return 'Total Vacaciones disponibles :';
	}else if($mostrar==0){
		return '';
	}
...
<head>
	...
	function mostrar(){
		//alert('asd');
		var idMostrar = document.getElementById("mostrar");
		var val = idMostrar.options[idMostrar.selectedIndex].value;
	
		
		if(val==1){
			alert('Los trabajadores ahora verán su saldo de vacaciones');
			
			window.location.replace("http://intranet.lanalessandri.cl/ematix/mostrar.php");
			
		}else if(val==0){
			alert('Los trabajadores ahora no verán su saldo de vacaciones');
			window.location.replace("http://intranet.lanalessandri.cl/ematix/noMostrar.php");
    ...
    <div class="cuerpo">
    	<table style="width:90%;">
            ...
            <tr>
            	<td style="text-align:right; width:200px; height:30px;"><label style="margin-right:10px;"><?php echo textSaldoVacaciones(); ?></label></td>
                <td style="text-align:left;"><?php echo saldoVacaciones(); ?></td>
				 <td colspan="2"style="text-align:right;">
					<?php
						if($tusuario == 2){
							echo 'Mostrar saldo:';
							echo '<select name="mostrar" id="mostrar" onchange="mostrar()">';
